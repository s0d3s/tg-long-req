...
/**/$BOT_THIS_TOKEN 		=		'xxxx:yyyy';						//токен выднный @BOTFATHER-ом
	...
	if($tgreq->ReqCheck()){
		...
		if($text != 'CANCEL') {//Упростим структуру, теперь из любого меню, выбрав 'CANCEL', мы вернёмся в главное меню
			...
			/*Дабы упорядочить "общение" с телеграммом, следует оформить результат функций-обработчиков следующи образом: 
					return array(
									'txt'				=> 'text for answer',
								OPT	'kbrd_mrkp' 		=> 'some keyboard list',
								OPT	'output_disabled'	=> 'if true, message doesnt be to send',
								OPT	'photo_arr'			=> array('photo_type'=>'real OR id', 0=>'array of photos path OR tg photo id'),
								OPT	'txt_arr'			=> array(0=>'caption for each elem in photo_arr')
								)
			*/
